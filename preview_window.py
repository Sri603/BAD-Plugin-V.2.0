...
class PreviewWindow(QtWidgets.QDialog, FORM_CLASS):
    ...
    def update_seed_threshold_preview(self, slider_value):
        ...
        thresholded = np.where(data >= threshold, data, 0)

        min_val = np.min(thresholded)
        max_val = np.max(thresholded)
        scale = 255 / (max_val - min_val + 1e-6)
        normalized = ((thresholded - min_val) * scale).clip(0, 255).astype(np.uint8)

        h, w = normalized.shape
        img = QImage(normalized.data, w, h, w, QImage.Format_Grayscale8).convertToFormat(QImage.Format_ARGB32)

        for y in range(h):
            for x in range(w):
                gray = QColor(img.pixel(x, y)).red()  # red==green==blue in grayscale
